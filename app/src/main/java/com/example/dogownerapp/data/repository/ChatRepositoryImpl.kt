...
class ChatRepositoryImpl @Inject constructor(
    ...
    override suspend fun sendFromOwner(specId: String, message: Message) {
        /*val docRef = firestore.collection("users").document(userId).collection("chats").document(specId)
        docRef.get().addOnSuccessListener { document ->
            if (document != null && document.exists()) {
                Log.d("Firestore", "Документ существует!")
            } else {
                UpdateNames()
            }
        }.addOnFailureListener { exception ->
            Log.w("Firestore", "Ошибка получения документа: ", exception)
        }*/
    ...
    private fun hasDialog(ownerId: String, specId: String) : Flow<Boolean> = callbackFlow {
        val docRef = firestore.collection("users").document(ownerId).collection("chats").document(specId)
        docRef.get().addOnSuccessListener { document ->
            if (document != null && document.exists()) {
                trySend(true)
            } else {
